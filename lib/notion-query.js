...
export const DB = {
  ...
  notificaciones:  '36e7ab7f-975e-8133-bb37-c4a24ba9d559',
};
...
export async function notifyNotion(apiKey, { tipo, titulo, detalle = '', origen = 'master-brain' }) {
  const props = {
    Título:  { title: [{ text: { content: titulo.slice(0, 100) } }] },
    Tipo:    { select: { name: tipo } },
    Origen:  { select: { name: origen } },
    Fecha:   { date: { start: new Date().toISOString() } },
  };
  if (detalle) props.Detalle = { rich_text: [{ text: { content: detalle.slice(0, 500) } }] };

  try {
    const res = await fetch('https://api.notion.com/v1/pages', {
      method: 'POST',
      headers: notionHeaders(apiKey),
      body: JSON.stringify({ parent: { database_id: DB.notificaciones }, properties: props })
    });
    return res.json();
  } catch (e) {
    console.error('notifyNotion error:', e.message);
    return null;
  }
}
