...
async function callGroq({ system, messages, tools, max_tokens, model }) {
  ...
  const doRequest = async (withTools) => {
    const body = {
      model: model || GROQ_MODEL,
      messages: toOpenAIMessages(system, messages),
      max_tokens: max_tokens || 1024
    };
    const openaiTools = withTools ? toOpenAITools(tools) : null;
    if (openaiTools) body.tools = openaiTools;
    const signal = AbortSignal.timeout(25000);
    ...
    if (!res.ok) {
      const errText = await res.text();
      const err = new Error(`Groq ${res.status}: ${errText.slice(0, 200)}`);
      err.isToolFail = errText.includes('tool_use_failed');
      throw err;
    }
  ...
  try {
    ...
    // tool_use_failed = model generated malformed tool call — retry as plain text
    if (err.isToolFail) return await doRequest(false);
    throw err;
  }
}
...
export class LLMClient {
  constructor(opts = {}) {
    // Groq primero: quota Gemini free-tier agotada (mayo 2026)
  ...
  async _create(params) {
    ...
    for (const fn of order) {
      try {
        ...
        console.error(`[llm-client] ${fn.name} falló: ${err.message}`);
      }
    }
    throw new Error(`LLM providers fallaron — ${errors.join(' | ')}`);
  }
}
...
export default class AnthropicCompat {
  constructor(_opts = {}) {
    const client = new LLMClient({ preferred: 'groq' });
